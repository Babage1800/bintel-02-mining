...
def plot_price_distribution(df_products: pd.DataFrame) -> None:
    ...
    ax.set_title("Goods And Their Potential Earnings (CLOSE chart to continue)")

    # Set the x-axis and y-axis labels for clarity
    ax.set_xlabel("Amount To Be Charged Per Good")
    ax.set_ylabel("Number Of Goods With This Price")
...
def plot_sales_trend(df_sales: pd.DataFrame) -> None:
    ...
    plot_line(
        ...
        x="YearMonth",
        y="SaleAmount",
        title="Revenue in One Year",
        xlabel="Time Frame",
        ylabel="Dollars Generated",
    )
